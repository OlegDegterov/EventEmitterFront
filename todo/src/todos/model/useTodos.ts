...
interface TodoState {
  todos: Todo[];
  getTodos: (todoProcessors: TodosProcessor[]) => Todo[];
  addTodo: (text: string) => void;
  toggleTodo: (todoId: Todo["id"]) => void;
  deleteTodo: (todoId: Todo["id"]) => void;
}

export type TodosProcessor = (todos: Todo[]) => Todo[];

export const useTodos = create<TodoState>()((set, get) => ({
  todos: defaultState,
  getTodos: (todoProcessors: TodosProcessor[]) => {
    return todoProcessors.reduce((prev, curr) => curr(prev), get().todos);
  },
  addTodo: (text: string) => {
    set((prevState) => ({
      todos: [
